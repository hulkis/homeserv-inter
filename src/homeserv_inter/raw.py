...
"""
    ...
    def prepare_data(data_):
        data = data_.merge(equipement_df, how="left", on="INSTANCE_ID")
        data = data.merge(
            orga_df,
            how="left",
        ...
        ]].merge(contrat_history_df).query("CRE_DATE_GZL>=UPD_DATE"))
        ...
        data = data.merge(
            contrat_history_s, how="left").merge(
                nature_code_eau_chaude, how="left")
        data = data.merge(
            nature_code_energie, how="left").merge(
                nature_code_fonction, how="left")
        data = data.merge(
            nature_code_installation, how="left").merge(
